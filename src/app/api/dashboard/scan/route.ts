...
export async function POST(
  ...
  try {
    ...
    // Log but don't fail the request — the user still gets their scan results
...
async function persistScanResults(
  ...
  // Create query and citation records
  for (const queryResult of result.queries) {
    ...
    for (const platformResult of queryResult.results) {
      await prisma.citation.create({
        data: {
          clientId,
          queryId: query.id,
          platform: platformResult.platform as AiPlatform,
          cited: platformResult.cited,
